...
class parseMZDATA:
    ...
    def _parsePoints(self, scanData):
        ...
        # convert from binary
        mzData = numpy.frombuffer(mzData[: (len(mzData) // struct.calcsize(mzEndian + mzPrecision)) * struct.calcsize(mzEndian + mzPrecision)], dtype=mzEndian + mzPrecision)
        intData = numpy.frombuffer(intData[: (len(intData) // struct.calcsize(intEndian + intPrecision)) * struct.calcsize(intEndian + intPrecision)], dtype=intEndian + intPrecision)
    ...
    def startElement(self, name, attrs):
        ...
        # get other params
            ...
            # get total ion current
                ...
                try:
                    self.data[self.currentID]["totIonCurrent"] = float(paramValue)
                except ValueError:
                    pass
    ...
    def startElement(self, name, attrs):
        ...
        # get other params
            ...
            # get total ion current
                ...
                try:
                    self.data["totIonCurrent"] = float(paramValue)
                except ValueError:
                    pass
    ...
    def endElement(self, name):
        ...
        if name == "mzArrayBinary":
            ...
            if not self.data[self.currentID]["mzData"]:
                ...
                self.data[self.currentID]["mzData"] = "".join(
                    self.data[self.currentID]["mzData"]
                )
        ...
        # stop reading int data
            ...
            if not self.data[self.currentID]["intData"]:
                ...
                self.data[self.currentID]["intData"] = "".join(
                    self.data[self.currentID]["intData"]
                )
