...
class panelEnvelopeFit(wx.Frame, MakeModalMixin):
    """Envelope fit tool."""
    ...
    def makeControlbar(self):
        ...
        self.fitToPeaklist_radio = wx.RadioButton(
            panel, -1, "Peak list", style=wx.RB_GROUP
        )
    ...
    def onCalculate(self, evt):
        ...
        # check document
        if not self.currentDocument or not (
            self.currentDocument.spectrum.hasprofile()
            or self.currentDocument.spectrum.haspeaks()
        ):
            wx.Bell()
            return
    ...
    def updateCanvasProperties(self, refresh=True):
        ...
        self.spectrumCanvas.setMFunction(
            [None, "cross"][config.spectrum["showTracker"]]
        )
    ...
    def runEnvelopeFit(self):
        ...
        try:
            ...
            # get scales
            scaleMin = max(
                0, min(config.envelopeFit["scaleMin"], config.envelopeFit["scaleMax"])
            )
            scaleMax = max(
                0, max(config.envelopeFit["scaleMin"], config.envelopeFit["scaleMax"])
            )
