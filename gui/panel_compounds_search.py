...
class panelCompoundsSearch(wx.Frame, MakeModalMixin):
    ...
    def makeGUI(self):
        ...
        self.mainSizer.Add(
            self.compoundsList, 1, wx.EXPAND | wx.ALL, mwx.LISTCTRL_NO_SPACE
        )
    ...
    def onItemSendToMassCalculator(self, evt):
        ...
        if radical == "radical":
            self.parent.onToolsMassCalculator(
                formula=formula, charge=charge, agentFormula="e", agentCharge=-1
            )
        else:
            self.parent.onToolsMassCalculator(
    ...
    def onGenerate(self, evt=None):
        ...
        self.processing = threading.Thread(
            target=self.runGenerateIons, kwargs={"compounds": compounds}
        )
            ...
            for name, compound in sorted(compounds.items()):
                ...
                for z in range(1, maxCharge):
                    ...
                    self.currentCompounds.append(
                        [name, mz, z * polarity, None, compound.expression, None, []]
                    )

                    # radicals
                    if config.compoundsSearch["radicals"]:
                        mz = compound.mz(
                            z * polarity, agentFormula="e", agentCharge=-1
