...
class envfit:
    ...
    def envelope(self, points=10):
        ...
        profile = mod_pattern.profile(
            isotopes, fwhm=self.fwhm, points=points, model=self.peakShape
        )
    ...
    def _initRange(self):
        ...
        self.mzrange[0] -= self.mzrange[0] * 0.001
        self.mzrange[1] += self.mzrange[1] * 0.001
        ...
        for x in sorted(self.models.keys()):
            ...
            if reset or pattern == []:
                pattern = compound.pattern(
                    fwhm=self.fwhm, charge=self.charge, real=False
                )
                self.models[x][1] = pattern

            # calculate model profile
            profile = mod_pattern.profile(
                pattern, fwhm=self.fwhm, raster=raster, model=self.peakShape
            )
    ...
    def _alignData(self):
        ...
        # make total envelope
        profile = mod_pattern.profile(
            isotopes, fwhm=self.fwhm, points=10, model=self.peakShape
        )

        # label peaks in profile
        peaklist = mod_peakpicking.labelscan(
            profile, pickingHeight=0.95, relThreshold=0.01
        )
        ...
        for peak in peaklist:
            for point in self.data:
                ...
                if abs(error) <= tolerance:
                    if (
                        calibrants
                        and calibrants[-1][0] == peak.mz
                        and calibrants[-1][1] < point[1]
                    ):
        ...
        for x in range(len(data)):
            ...
            chisq_deriv = list(
                map(
                    lambda a, b: a + b,
                    chisq_deriv,
                    list(map(lambda x, f=differences[x] * 2: f * x, deriv)),
                )
            )
