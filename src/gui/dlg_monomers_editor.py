...
class dlgMonomersEditor(wx.Dialog):
    ...
    def makeItemsList(self):
        ...
        self.itemsList = mwx.sortListCtrl(
            self, -1, size=wx.Size(871, 250), style=mwx.LISTCTRL_STYLE_MULTI
        )
    ...
    def makeItemEditor(self):
        ...
        self.itemSearch_value = wx.TextCtrl(
            self, -1, "", size=wx.Size(200, -1), style=wx.TE_PROCESS_ENTER
        )
        self.itemSearch_value.Bind(wx.EVT_TEXT, self.onSearch)

        itemAbbr_label = wx.StaticText(self, -1, "Abbr.:")
        self.itemAbbr_value = wx.TextCtrl(self, -1, "", size=wx.Size(200, -1))
        ...
        self.itemCategory_value = wx.TextCtrl(self, -1, "", size=wx.Size(200, -1))

        itemFormula_label = wx.StaticText(self, -1, "Formula:")
        self.itemFormula_value = mwx.formulaCtrl(self, -1, "", size=wx.Size(150, -1))
        ...
        self.itemLossMoMass_value = wx.TextCtrl(self, -1, "", size=wx.Size(150, -1))
        ...
        grid.Add(
            itemSearch_label, (0, 0), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        ...
        grid.Add(
            itemCategory_label, (3, 0), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        grid.Add(self.itemCategory_value, (3, 1), flag=wx.ALIGN_CENTER_VERTICAL)

        grid.Add(
            itemFormula_label, (0, 3), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        grid.Add(self.itemFormula_value, (0, 4), flag=wx.ALIGN_CENTER_VERTICAL)
        grid.Add(
            itemMoMass_label, (1, 3), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        grid.Add(self.itemMoMass_value, (1, 4), flag=wx.ALIGN_CENTER_VERTICAL)
        grid.Add(
            itemAvMass_label, (2, 3), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        grid.Add(self.itemAvMass_value, (2, 4), flag=wx.ALIGN_CENTER_VERTICAL)
        grid.Add(
            itemLossMoMass_label, (3, 3), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        grid.Add(self.itemLossMoMass_value, (3, 4), flag=wx.ALIGN_CENTER_VERTICAL)

        grid.Add(
            itemLosses_label, (0, 6), flag=wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL
        )
        ...
        for i in self.itemsList.getSelected():
            ...
            if name not in self.used:
                ...
                dlg = mwx.dlgMessage(
                    ...
                    message="Remove the monomer from all of your documents first.",
                )
                dlg.ShowModal()
                dlg.Destroy()
    ...
    def updateItemsMap(self):
        ...
        for abbr, monomer in sorted(mspy.monomers.items()):
            ...
            if search and not (
                all([x in monomer.abbr.lower() for x in search])
                or all([x in monomer.name.lower() for x in search])
            ):
                continue
