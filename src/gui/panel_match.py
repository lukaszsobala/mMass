...
class panelMatch(wx.Frame, MakeModalMixin):
    ...
    def makeGUI(self):
        ...
        self.mainSizer.Add(
            self.summaryList, 1, wx.EXPAND | wx.ALL, mwx.LISTCTRL_NO_SPACE
        )
    ...
    def makeErrorCanvas(self):
        ...
        self.errorCanvas.setProperties(showXPosBar=False)
        self.errorCanvas.setProperties(showYPosBar=False)
    ...
    def onUnitsChanged(self, evt=None):
        ...
        if config.match["units"] == "ppm":
            for x in range(len(self.currentErrors)):
                self.currentErrors[x][1] = self.currentErrors[x][1] / (
                    self.currentErrors[x][0] / 1000000
                )
            ...
            for pIndex, peak in enumerate(self.currentPeaklist):
                for x, item in enumerate(self.currentData):
                    ...
                    if (
                        ...
                        error = mspy.delta(
                            peak.mz, item[massCol], config.match["units"]
                        )
    ...
    def makeMatchSummary(self):
        ...
        # get searched peaks
        self.currentSummary.append(
            ("Number of peaks searched", len(self.currentPeaklist))
        )
        ...
        if self.currentModule == "digest":
            ...
            for item in self.currentData:
                if item[-1]:
                    sumPeptides.append(
                        [item[6].history[-1][1] + 1, item[6].history[-1][2]]
                    )
            coverage = mspy.coverage(
                sumPeptides, self.currentSummaryData["sequenceLength"]
            )
            value = "%0.f %s" % (coverage, "%")
            self.currentSummary.append(
                ("Sequence length", self.currentSummaryData["sequenceLength"])
            )
