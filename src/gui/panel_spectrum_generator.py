...
class panelSpectrumGenerator(wx.Frame, MakeModalMixin):
    ...
    def makeToolbar(self):
        ...
        self.peakShape_choice = wx.Choice(
            panel,
            -1,
    ...
    def onApply(self, evt):
        ...
        if self.currentDocument.spectrum.hasprofile():
            title = (
                "Do you really want to apply generated spectrum\nto current document?"
            )
    ...
    def updateCanvasProperties(self, refresh=True):
        ...
        if len(self.spectrumContainer):
            self.spectrumContainer[0].setProperties(
    ...
    def updateSpectrumOverlay(self):
        ...
        if self.currentProfile is None or not config.spectrumGenerator["showOverlay"]:
            ...
            self.parent.updateTmpSpectrum(
                self.currentProfile, flipped=config.spectrumGenerator["showFlipped"]
            )
