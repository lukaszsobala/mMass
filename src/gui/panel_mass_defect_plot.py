...
class panelMassDefectPlot(wx.Frame):
    ...
    def makePlotCanvas(self):
        ...
        self.plotCanvas.setProperties(showXPosBar=False)
        self.plotCanvas.setProperties(showYPosBar=False)
    ...
    def calcDataPoints(self, peaks, polarity=1):
        ...
        for peak in peaks:
            ...
            if not config.massDefectPlot["ignoreCharge"] and peak[1]:
                mass = mspy.mz(
                    peak[0], 1 * polarity, peak[1], agentFormula="H", agentCharge=1
                )
            ...
            if config.massDefectPlot["xAxis"] == "mz":
                ...
                mass = mspy.nominalmass(
                    peak[0] * kendrickFormula.nominalmass() / kendrickFormula.mass(0)
                )
