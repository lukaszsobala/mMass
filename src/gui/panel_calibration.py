...
class panelCalibration(wx.Frame, MakeModalMixin):
    ...
    def makeReferencesPanel(self):
        ...
        self.toleranceUnits_label = wx.StaticText(
            ctrlPanel, -1, config.calibration["units"]
        )
        ...
        if group and group in libs.references:
            ...
            for item in libs.references[group]:
                self.currentReferences.append(
                    [item[0], item[1], None, None, None, None, True]
                )
    ...
    def updateErrorPlot(self):
        ...
        if self.currentReferences:
            ...
            if (
                ...
                and self.currentDocument
                and self.currentDocument.spectrum.peaklist
            ):
        ...
        self.errorCanvas.setProperties(
            yLabel="error (%s)" % (config.calibration["units"])
        )
    ...
    def calcCalibration(self):
        ...
        if (model == "linear" and len(points) >= 1) or (
            model == "quadratic" and len(points) >= 3
        ):
