...
class mMass(wx.App):
    ...
    def OnInit(self):
        ...
        if options.session or options.documents:
            paths = [options.session] if options.session else []
            self.frame.onDocumentDropped(paths=paths + options.documents)
    ...
    def MacOpenFile(self, fileName):
        ...
        self.frame.onDocumentDropped(paths=[path])
...
def run(options):
    """Start the GUI and open the documents given as cli.StartupOptions."""
